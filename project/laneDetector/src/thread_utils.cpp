                    ...
                    const ProcessParams& processParams )
{
    ...
    {
        LogFatal( "pthread_setname_np failure [%s] in thread %s.", strerror( errno ), threadName.c_str() );
    }

    if( pthread_attr_destroy( &threadAttr ) )
    {
        LogFatal( "pthread_attr_destroy failure [%s] in thread %s.", strerror( errno ), threadName.c_str() );
    }
                    ...
                    const ProcessParams& processParams )
{
    ...
    {
        LogFatal( "Invalid schedule policy and/or priority in thread %s.", threadName.c_str() );
    }
}
                                  ...
                                  pthread_attr_t& threadAttr )
{
    ...
    else
    {
        if( cpu >= NUM_CPUS )
        {
            LogFatal( "Invalid CPU # (affinity not set) in thread %s.", threadName.c_str() );
        }
